...
class PropertyLabels {
	...
	private static void putEpdLabels() {
		...
		putBasicLabels(type);
		put(type, "urn", M.Urn);
		put(type, "manufacturer", M.DeclarationOwnerManufacturer, ModelType.ACTOR);
		...
		put(type, "epdType", M.EpdType);
		put(type, "validFrom", M.PublicationDate);
		put(type, "validUntil", M.ValidUntil);
		put(type, "location", M.Location);
		put(type, "originalEpd", M.OriginalEpd);
		put(type, "manufacturing", M.ManufacturingDescription);
		put(type, "productUsage", M.ProductUsageDescription);
		put(type, "useAdvice", M.UseAdvice);
		put(type, "registrationId", M.RegistrationNumber);
		put(type, "dataGenerator", M.DataGenerator);
