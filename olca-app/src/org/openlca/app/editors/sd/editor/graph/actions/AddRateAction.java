...
/**
 * Action to add a Rate node to the graph.
 */
public class AddRateAction extends Action {

	public static final String ID = "sd.graph.addRate";
	...
	public AddRateAction(SdGraphEditor editor) {
		...
		setId(ID);
		setText("Add Rate");
		setToolTipText("Add a new rate (flow) variable");
	...
	public void run() {
		...
		var name = "Rate_" + rateCounter++;
		var node = new SdNode(SdNodeType.RATE, name);

		// Place the new node at a default position
		var bounds = new Rectangle(new Point(150, 100), node.getDefaultSize());
