...
public class GraphAnimation {
	...
	static boolean playbackState(final Connection conn) {
		if (!PLAYBACK) {
			return false;
		}

		final PointList list1 = (PointList) initialStates.get(conn);
		final PointList list2 = (PointList) finalStates.get(conn);
		if (list1 == null) {
			conn.setVisible(false);
			return true;
		}
		if (list1.size() == list2.size()) {
			final Point pt1 = new Point(), pt2 = new Point();
	...
	static boolean playbackState(final IFigure container) {
		if (!PLAYBACK) {
			return false;
		}
	...
	static void recordFinalState(final Connection conn) {
		final PointList points1 = (PointList) initialStates.get(conn);
		final PointList points2 = conn.getPoints().getCopy();

		if (points1 != null && points1.size() != points2.size()) {
			final Point p = new Point(), q = new Point();

			final int size1 = points1.size() - 1;
			final int size2 = points2.size() - 1;
	...
	static void recordFinalState(final IFigure child) {
		if (child instanceof Connection) {
			recordFinalState((Connection) child);
			return;
		}
		final Rectangle rect2 = child.getBounds().getCopy();
		final Rectangle rect1 = (Rectangle) initialStates.get(child);
		if (rect1.isEmpty()) {
			rect1.x = rect2.x;
			rect1.y = rect2.y;
	...
	static void recordInitialState(final IFigure container) {
		if (!RECORDING) {
			return;
		}
	...
	static void swap() {
		final Map<IFigure, Translatable> temp = finalStates;
	...
	public static boolean captureLayout(IFigure root) {
		...
		root.validate();
		final Iterator<IFigure> iter = initialStates.keySet().iterator();
		if (!iter.hasNext()) {
	...
	public static void recordInitialState(final Connection connection) {
		if (!RECORDING) {
			return;
		}
		final PointList points = connection.getPoints().getCopy();
		if (points.size() == 2
